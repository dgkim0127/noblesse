-- This schema is for local PostgreSQL development/testing.
-- POS APK must not connect directly to PostgreSQL.
-- Production writes must go through an API layer.
-- Do not commit database credentials.
-- TODO: Add RLS policies after production access roles are finalized.
...
create index if not exists idx_sync_logs_source_status on sync_logs(source_system, sync_status);
create index if not exists idx_pos_import_batches_source_status on pos_import_batches(source_system, status);
