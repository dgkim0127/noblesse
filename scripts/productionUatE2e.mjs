...
export const REQUIRED_ENV = [
  "NOBLESSE_PRODUCTION_UAT_ALLOW",
  "VITE_API_BASE_URL",
  "VITE_FIREBASE_API_KEY",
  "NOBLESSE_PROD_UAT_BUYER_EMAIL",
  "NOBLESSE_PROD_UAT_BUYER_PASSWORD",
  "NOBLESSE_PROD_UAT_ADMIN_EMAIL",
  "NOBLESSE_PROD_UAT_ADMIN_PASSWORD"
];

const EXACT_PRODUCTION_API_BASE_URL = "https://noblesse.web.app/api";
const FORBIDDEN_BASE_URL_PATTERNS = [
  /staging/i,
  /noblesse-admin-backend/i,
  /staging-api/i
];
...
function logStep(label, facts = {}) {
  ...
  console.log(`[production-uat-e2e] ${label}${entries.length ? ` ${entries.join(" ")}` : ""}`);
}
...
export function validateProductionUatEnv(env = process.env) {
  const missing = REQUIRED_ENV.filter((key) => !env[key]);
  if (missing.length) {
    throw new Error(`Missing required env: ${missing.join(", ")}`);
  }

  if (env.NOBLESSE_PRODUCTION_UAT_ALLOW !== "YES") {
    throw new Error("NOBLESSE_PRODUCTION_UAT_ALLOW must be YES");
  }
  ...
  try {
    ...
    throw new Error("VITE_API_BASE_URL must be an absolute production API URL");
  }
  ...
  if (baseUrl !== EXACT_PRODUCTION_API_BASE_URL) {
    throw new Error("VITE_API_BASE_URL must be exactly https://noblesse.web.app/api");
  }
  if (FORBIDDEN_BASE_URL_PATTERNS.some((pattern) => pattern.test(baseUrl))) {
    throw new Error("VITE_API_BASE_URL must not use a staging API URL");
  }

  return {
    apiBaseUrl: baseUrl,
    firebaseApiKey: env.VITE_FIREBASE_API_KEY.trim(),
    buyerEmail: env.NOBLESSE_PROD_UAT_BUYER_EMAIL,
    buyerPassword: env.NOBLESSE_PROD_UAT_BUYER_PASSWORD,
    adminEmail: env.NOBLESSE_PROD_UAT_ADMIN_EMAIL,
    adminPassword: env.NOBLESSE_PROD_UAT_ADMIN_PASSWORD
  };
}
...
function createSyntheticInput() {
  ...
  return {
    label: `PROD_UAT_35A_${stamp}`,
    categoryKey: `prod-uat-35a-${stamp.toLowerCase()}`,
    productCode: `PRODUAT35A-${short}`,
    productName: `PROD UAT 35A Product ${short}`,
    updatedCategoryName: `PROD UAT 35A Updated Category ${short}`,
    updatedProductMaterial: `PROD UAT Updated Surgical Steel ${short}`,
    updatedProductName: `PROD UAT 35A Updated Product ${short}`
  };
}
...
async function main() {
  const env = validateProductionUatEnv();
  ...
  try {
    ...
    try {
      await api("/admin/dashboard", { token: buyerToken });
      throw new Error("Buyer admin endpoint access unexpectedly succeeded");
    } catch (error) {
      const message = error?.message || "";
      if (!message.includes('"status":401') && !message.includes('"status":403')) {
        throw error;
      }
      logStep("buyer-admin-access-rejected", { ok: "yes" });
    }
    ...
    const productBody = await api("/admin/products", {
      ...
      body: {
        ...
        sortOrder: 9999,
        descriptionEn: "Synthetic production UAT catalog product."
      }
    });
    ...
    const quoteBody = await api("/admin/quotes", {
      ...
      body: {
        ...
        leadTime: "Production UAT",
        shippingNote: "Synthetic production UAT only",
    ...
    const buyerTokenFresh = await firebaseSignIn({
      apiKey: env.firebaseApiKey,
      email: env.buyerEmail,
      password: env.buyerPassword,
      label: "buyer-fresh-session"
    });
    const freshBuyerInquiryBody = await api(`/buyer/inquiries/${encodeURIComponent(created.inquiryId)}`, {
      token: buyerTokenFresh
    });
    const freshBuyerInquiry = unwrapData(freshBuyerInquiryBody).inquiry;
    if (!freshBuyerInquiry?.id) {
      throw new Error("Fresh buyer session remote read-back failed");
    }
    logStep("fresh-buyer-session-readback", { ok: "yes", status: freshBuyerInquiry?.status || "unknown" });
    ...
    logStep("complete", { result: "PRODUCTION_UAT_E2E_GO" });
  } catch (error) {
...
if (isCliRun) {
  main().catch((error) => {
    fail(error?.message || "Unexpected production UAT failure");
  });
}
