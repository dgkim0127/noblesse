...
test('authenticated commerce state loads admin catalog prices through admin read API', () => {
  ...
  assert.match(source, /const adminPriceRowSafetyLimit = 10_000/)
  assert.match(source, /Admin price pagination returned a repeated offset/)
  assert.match(source, /Admin price pagination exceeded the safety limit/)
  assert.doesNotMatch(source, /adminPricePageCap/)
  ...
  assert.match(source, /const adminPriceBooksByProduct = useMemo/)
  assert.match(source, /grouped\.set\(productId, getAdminPriceBooksForProduct\(productPricesForId, productId\)\)/)
  assert.match(source, /adminPriceBooksByProduct\.get\(productId\) \|\| \[\]/)
