...
test('quote density preview creates 15 safe display-only sample items', () => {
  ...
  assert.match(quotePage, /\{dirty && editable\s*\? <button className="primary-action"/)
  assert.match(quotePage, /workflowStatus === 'finalized'/)
  assert.match(quotePage, /openPublishDialog/)
...
test('online quote workflow separates internal finalization from customer publication', () => {
  ...
  const adminPosQuoteCopy = read('src/pages/admin/adminPosQuoteCopy.js')
  const adminQuoteFeature = `${adminQuote}\n${adminQuoteCopy}\n${adminPosQuoteCopy}`
  const buyerQuotes = read('src/pages/MyInquiriesPage.jsx')

  assert.match(adminQuote, /savePosQuotePicking/)
  assert.match(adminQuote, /finalizePosQuote/)
  assert.match(adminQuote, /publishPosQuote/)
  assert.match(adminQuote, /posT\.publishButton/)
  assert.match(adminQuote, /posT\.publishedNotice/)
  assert.match(adminQuote, /posT\.finalizeDescription/)
  ...
  assert.match(adminQuote, /workflowSteps = \['received', 'picking', 'finalized', 'published'\]/)
  ...
  assert.doesNotMatch(adminQuote, /issueQuote/)
  assert.doesNotMatch(buyerQuotes, /decideInquiryQuote/)
  assert.match(buyerQuotes, /const isOnlineQuote = Boolean\(issuedQuote\?\.isOnlineQuote\)/)
  assert.match(buyerQuotes, /!isOnlineQuote && <ol className="buyer-quote-workflow"/)
  assert.match(buyerQuotes, /!isOnlineQuote && <p className="buyer-quote-external-payment"/)
