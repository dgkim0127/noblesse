...
create table if not exists public.product_prices (
  ...
  fx_anchor_snapshot_id uuid,
  fx_last_reviewed_at timestamptz,
  fx_last_applied_at timestamptz,
  updated_at timestamptz default now(),
...
create index if not exists idx_product_prices_fx_managed on public.product_prices(fx_managed, market, currency);
...
create index if not exists idx_fx_review_runs_created_at on public.fx_review_runs(created_at desc);
create unique index if not exists idx_fx_price_drafts_pending_unique on public.fx_price_drafts(product_id, target_market, target_currency) where status = 'pending';
create index if not exists idx_fx_price_drafts_status_created_at on public.fx_price_drafts(status, created_at desc);
