...
export function createBuyerRegistrationQueries(pool) {
  return {
    async registerBuyer(identity, input, context = {}) {
      ...
      const registerWithColumnSupport = async (columnSupport) => {
        ...
        try {
          ...
          const existingUserResult = await client.query(
            `
              select id, auth_uid, email, role, status,
                ${columnSupport.hasAccountStatus ? "account_status" : "null::text as account_status"}
              from public.users
          ...
          if (existingUser) {
            const existingBuyerResult = await client.query(
              `
                select id,
                  ${columnSupport.hasVerificationStatus ? "verification_status" : "null::text as verification_status"}
                from public.buyers
                where user_id = $1
                limit 1
                for update
          ...
          const userResult = existingUser
            ? await client.query(
                columnSupport.hasAccountStatus ? `
                  update public.users
                  set email = $2,
                      status = 'pending',
                ...
                ` : `
                  ...
                  set email = $2,
                      ...
                      updated_at = now()
                  where id = $1
                  returning id, auth_uid, email, role, status, null::text as account_status
                `,
          ...
          const buyerResult = await client.query(
            columnSupport.hasVerificationStatus ? `
