...
test("createInquiry accepts KRW numeric strings with zero fractional scale", async () => {
  const calls = [];
  const client = {
    async query(sql, params) {
      calls.push({ sql: String(sql), params });
      if (/join public\.product_prices/i.test(sql)) {
        return {
          rows: [{
            product_id: "product-1",
            product_code: "NB-4WAY-GREEN-CLOVER-BARBELL",
            name_en: "4-Way Green Clover Barbell",
            name_ko: "4방 초록클로버 바벨",
            category_id: "barbell",
            material: "Surgical Steel",
            id: "price-1",
  ...
  const inquiry = await createBuyerInquiryQueries(pool).createInquiry(
    ...
    {
      requestMemo: "Check KRW numeric scale",
      items: [{ productCode: "NB-4WAY-GREEN-CLOVER-BARBELL", color: "오알", quantity: 1 }]
    }
  );
  ...
  assert.equal(inquiry.market, "KR");
  assert.equal(inquiry.currency, "KRW");
  assert.equal(inquiry.estimatedTotal, 1800);
  assert.deepEqual(inquiryInsert.params.slice(2, 7), ["KR", "KRW", 1, 1, 1800]);
  assert.equal(itemInsert.params[10], 1800);
  assert.equal(itemInsert.params[11], 1800);
  assert.ok(calls.some((call) => call.sql === "commit"));
});
