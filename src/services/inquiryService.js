...
// The hybrid gateway returns buyer-specific approved prices. These values are
// display-only here; the Cloud Function recalculates every line on submission.
export const buildInquiryRowsFromApprovedPrices = (inquiryItems, products, getPrice) => inquiryItems.map((item) => {
  const product = products.find((candidate) => candidate.productId === item.productId)
  const price = getPrice(item.productId)
  const option = getDefaultOption(product, item)
  if (!product || !price || price.approvedPrice === null || price.approvedPrice === undefined) return null
  ...
  return {
    ...
    size: option.size,
    moq: price.moq,
    priceSnapshot: price.approvedPrice,
    priceSnapshotMinor: price.approvedPriceMinor,
    subtotal: price.approvedPrice * item.quantity,
    subtotalMinor: price.approvedPriceMinor * item.quantity,
    tone: product.tone,
  }
}).filter(Boolean)
