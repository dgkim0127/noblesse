...
export function ApprovalPendingPage() {
  const { buyer, isApproved, isGuest, isHybridMode, isPending, setViewerState } = useCommerce()
  ...
  return <main className="content auth-page">
    <section className="account-panel auth-panel">
      ...
      {!isHybridMode && !isApproved && <button className="text-action preview-action" type="button" onClick={() => setViewerState('approved')}>개발 미리보기: 회원가 보기</button>}
    </section>
  </main>
}
