...
const registerCopy = {
  kr: {
    ...
    approvalBody: '거래처 정보는 담당자가 직접 확인합니다. 확인 전까지 가격과 견적 요청 기능은 잠겨 있습니다.',
  ...
  en: {
    ...
    approvalBody: 'Trade information is reviewed manually. Prices and quote request features remain locked until review is complete.',
  ...
  cn: {
    ...
    helper: '提交貿易資訊後，工作人員會確認並提供交易條件、最小數量和詢價請求說明。',
    noticeTitle: '咨询前请确认',
    noticeItems: [
      '贸易信息将由工作人员确认。',
      '确认后可提供交易条件说明。',
    ...
    approvalBody: '貿易資訊將由工作人員確認。確認前，交易條件和詢價請求功能將保持鎖定。',
...
const localizedAgreementSections = {
  terms_of_service: {
    kr: [
      ...
      ['제2조 용어의 정의', ['이용자는 사이트에 접속하거나 서비스를 이용하는 모든 방문자와 회원을 의미합니다.', '회원은 회원 가입 절차를 통해 정보를 제출하고 귀족의 카탈로그 및 문의 서비스를 이용하는 개인 또는 사업자 담당자를 의미합니다.', '승인 회원은 귀족이 거래 가능 여부를 확인한 뒤 회원가, MOQ, 견적 요청 등 일부 기능 접근을 허용한 회원을 의미합니다.', '견적 요청은 상품, 옵션, 수량, 메모를 기준으로 거래 검토를 요청하는 절차이며 최종 주문이나 대금 확정이 아닙니다.']],
  ...
  buyer_terms: {
    kr: [
      ...
      ['제3조 회원 상태', ['회원 상태는 비회원, 확인 중, 승인 회원, 관리자 등으로 구분될 수 있습니다.', '확인 중인 회원은 상품 카탈로그를 볼 수 있으나 회원가, 견적 요청, 거래 조건 안내는 제한될 수 있습니다.']],
...
const localizedAgreementDetailOverrides = {
  terms_of_service: {
    ...
    jp: [
      ...
      ['第2条 用語の定義', ['利用者とは、サイトにアクセスし、またはサービスを利用するすべての訪問者および会員をいいます。', '会員とは、会員登録手続を通じて情報を提出し、貴族のカタログおよびお問い合わせサービスを利用する個人または事業者担当者をいいます。', '承認バイヤーとは、貴族が取引可能性を確認した後、会員価格、MOQ、見積依頼など一部機能へのアクセスを許可した会員をいいます。', '見積依頼とは、商品、オプション、数量、メモを基準に取引検討を依頼する手続であり、最終取引確定ではありません。']],
  ...
  buyer_terms: {
    en: [
      ['Article 1 Purpose', 'These terms define detailed standards for Noblesse wholesale buyer review, member price access, MOQ, quote request, and trade terms guidance.'],
      ...
      ['Article 3 Member Status and Access', ['Member status may be divided into guest, under review, approved buyer, and admin.', 'Members under review may browse the product catalog, but member price, quote request, and trade terms guidance may be limited.']],
    ...
    jp: [
      ...
      ['第3条 会員状態およびアクセス', ['会員状態はゲスト、審査中、承認バイヤー、管理者などに区分されることがあります。', '審査中の会員は商品カタログを閲覧できますが、会員価格、見積依頼、取引条件案内は制限される場合があります。']],
