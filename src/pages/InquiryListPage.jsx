...
export function InquiryListPage() {
  ...
  return <main className="content"><div className="page-title"><div><p>INQUIRY LIST</p><h1>Selected piercing</h1></div><span>{inquiryRows.length} items / {totalQuantity} pcs</span></div><section className="inquiry-layout"><div className="inquiry-items">{inquiryRows.map((row) => {
    const option = { color: row.color, size: row.size }
    const rowKey = `${row.productId}-${row.color}-${row.size}`
    return <article className="inquiry-row" key={rowKey}><div className={`mini-image tone-${row.tone}`}><span className="jewel-shape" />{row.thumbnailUrl && <img src={row.thumbnailUrl} alt={row.imageAlt?.en ?? row.productName} loading="lazy" width="300" height="300" onError={(event) => { event.currentTarget.hidden = true }} />}</div><div className="inquiry-product"><strong>{row.productName}</strong><span>{row.productCode}</span><small>{row.material} / {row.color} / {row.size}</small><small>MOQ {row.moq} / Approved Buyer Price {formatMoney(row.priceSnapshot, buyer.currency)}</small></div><div className="quantity"><button type="button" aria-label="Decrease quantity" onClick={() => updateInquiryQuantity(row.productId, row.quantity - row.moq, option)}><Minus size={14} /></button><input value={row.quantity} type="number" min={row.moq} step={row.moq} onChange={(event) => updateInquiryQuantity(row.productId, event.target.value, option)} /><button type="button" aria-label="Increase quantity" onClick={() => updateInquiryQuantity(row.productId, row.quantity + row.moq, option)}><Plus size={14} /></button></div><b>{formatMoney(row.subtotal, buyer.currency)}</b><button className="remove" type="button" aria-label="Remove item" onClick={() => removeInquiryItem(row.productId, option)}><Trash2 size={16} /></button></article>
