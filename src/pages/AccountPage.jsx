...
export function AccountPage() {
  ...
  const { buyer, buyerAccess, isAdmin, isApproved, isGuest, isHybridMode, isPending, logout, viewerState } = useCommerce()
  ...
  const logoutCurrentBuyer = async () => {
    await logout()
  ...
  return <main className="content">
    ...
    <section className="account-panel account-overview">
      ...
      {isAdmin && <div className="account-status-card"><ShieldCheck size={25} /><h2>관리자 권한</h2><p>바이어 승인, 견적 처리, 카탈로그와 시장별 가격을 관리할 수 있습니다.</p><div className="account-actions"><Link className="primary-action" to="/admin">관리자 사이트</Link><Link className="secondary-action" to={toLocalePath('/products')}>구매자 화면</Link></div></div>}
    </section>
    ...
    <section className="account-panel">
      ...
      <button className="text-action logout-action" type="button" onClick={() => { void logoutCurrentBuyer() }}><LogOut size={15} />{isHybridMode ? '로그아웃' : '목업 로그아웃'}</button>
    </section>
  </main>
}
