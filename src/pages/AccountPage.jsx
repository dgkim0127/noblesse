...
export function AccountPage() {
  ...
  const minimumRequestAmount = !isApproved ? '로그인 후 확인 가능' : formatMoney(buyer.minOrderAmount, buyer.currency)
  const profileRows = [
  ...
  return <main className="content">
    ...
    <section className="account-panel account-overview">
      {isGuest && <div className="account-status-card"><UserRound size={25} /><h2>비회원으로 둘러보는 중입니다.</h2><p>상품 카탈로그는 볼 수 있으며, 가격과 견적 기능은 로그인한 거래처 계정에서 바로 사용할 수 있습니다.</p><div className="account-actions"><Link className="primary-action" to={toLocalePath('/register')}>거래처 등록</Link><Link className="secondary-action" to={toLocalePath('/login')}>로그인</Link><Link className="secondary-action" to={toLocalePath('/products')}>상품 목록 보기</Link></div></div>}
      {isPending && <div className="account-status-card"><Clock3 size={25} /><h2>거래처 이용 상태 확인이 필요합니다.</h2><p>{buyer.companyName} 계정 정보를 확인할 수 없습니다. 등록 정보를 확인하거나 담당자에게 문의해주세요.</p><div className="account-actions"><Link className="primary-action" to={toLocalePath('/products')}>상품 목록 보기</Link><Link className="secondary-action" to={toLocalePath('/register')}>거래처 정보 수정</Link><a className="secondary-action" href="mailto:dgkim0127@gmail.com"><Mail size={15} />이메일 문의</a></div></div>}
