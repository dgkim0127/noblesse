...
export function CatalogCard({ product }) {
  ...
  return <article className="catalog-card">
    <Link className={`catalog-image tone-${product.tone}`} to={`/products/${product.productId}`} aria-label={product.nameEn}>
      <span className="jewel-shape" />
      {product.imageSet?.card && <img src={product.imageSet.card} alt={product.imageAlt?.en ?? product.nameEn} loading="lazy" width="600" height="600" onError={(event) => { event.currentTarget.hidden = true }} />}
