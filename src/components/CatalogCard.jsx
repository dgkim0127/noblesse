  ...
  return <article className="catalog-card">
    <div className="catalog-media">
      <Link className={`catalog-image tone-${product.tone}`} to={toLocalePath(`/products/${product.productId}`)} aria-label={productName}>
        ...
        {product.imageSet?.card && <img src={product.imageSet.card} alt={productAlt} loading="lazy" decoding="async" width="600" height="900" style={{ objectFit: 'cover', ...imagePresentationStyle(product.imageSet) }} onError={(event) => { event.currentTarget.hidden = true }} />}
